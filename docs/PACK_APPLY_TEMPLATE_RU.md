...
# Safety: проверить, что pack не содержит запрещённых путей
# (абсолютные пути, .., или .git/). Такие архивы не распаковываем.
if tar -tzf "$PACK" | grep -Eq '(^/|(^|/)\.\.(\/|$)|(^|/)\.git(/|$))'; then
  echo "Пак содержит запрещённые пути (.git/ или .. или абсолютные).";
  tar -tzf "$PACK" | grep -E '(^/|(^|/)\.\.(\/|$)|(^|/)\.git(/|$))' | head -n 50;
  false;
fi &&
...
# Удалить типовой мусор Python (игнорируется git, но может мешать проверкам)
...
- Если тебе прислали "полный zip репозитория" и внутри есть `.git/`, **не распаковывай его поверх своего репозитория**.
  - Для передачи полного снимка (без истории и без `.git/`) делай zip так: `git archive -o /media/nik/0C30B3CF30B3BE50/Загрузки/adaspeas.zip HEAD`.
  - Нужен именно инкрементальный pack.

Актуально на: 2026-02-08 21:40 MSK

## История изменений
| Дата/время (MSK) | Автор | Тип | Кратко | Commit/PR |
|---|---|---|---|---|
| 2026-02-08 21:40 MSK | ChatGPT | doc | Добавлено: pre-check содержимого pack до распаковки (запрет .git/ .. /); добавлена зачистка python cache (pytest/__pycache__/pyc) | |
