...
- Для кнопки “Назад” используем сохранённый в каталоге указатель на родителя (`parent_path`), чтобы не тащить “стек” состояний в FSM.

Реализация MVP (на сегодня):
- `nav:<id>` → бот достаёт элемент из SQLite, берёт `path`, делает `list_dir(path)` в хранилище и upsert’ит детей с `parent_path=path`.
- UI редактирует одно сообщение и не светит пути хранилища обычным пользователям.
