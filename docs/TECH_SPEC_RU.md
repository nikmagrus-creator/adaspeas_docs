...
Требования:
- синхронизация запускается по расписанию (например, раз в 10 минут) и/или вручную админом (MVP: ручной /sync);
...
- Нет фоновой периодической синхронизации каталога по расписанию (MVP: есть ручной `/sync`).
