# Контекст проекта Adaspeas (бот + автодеплой) — актуально на 2026-02-03 19:05 UTC

Актуально на: 2026-02-03 19:05 UTC
...
Важно:
- если вы меняли `deploy/Caddyfile` (или другие монтируемые конфиги) и эффект не виден, контейнер caddy нужно перезапустить:
  - `cd /opt/adaspeas && docker compose -f docker-compose.prod.yml restart caddy`
...
Применение архива (локально, Linux Mint):
- архив ожидается в `/media/nik/0C30B3CF30B3BE50/Загрузки`
- распаковка в репозиторий: `tar -xzf /media/nik/0C30B3CF30B3BE50/Загрузки/<archive>.tar.gz -C /projects/adaspeas`
...
- Папка, куда сохраняем архивы из чата: `/media/nik/0C30B3CF30B3BE50/Загрузки`
...
| 2026-02-03 19:05 UTC | Nikolay | ops | Зафиксировано: после деплоя перезапускать caddy, путь загрузок для архивов | runbook/процесс | |
