# Контекст проекта Adaspeas (бот + автодеплой) — актуально на 2026-02-03 19:10 UTC

Актуально на: 2026-02-03 19:10 UTC
...
Если вместо этого 404 от aiohttp, значит caddy не перезапустился после изменения Caddyfile.
Решение:
- на VPS выполнить: `docker compose -f docker-compose.prod.yml restart caddy`
Опционально (если хочешь редирект делать на уровне приложения): добавить handler в aiohttp.
