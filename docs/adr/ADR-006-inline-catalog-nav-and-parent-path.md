...
## Consequences
Плюсы:
- `callback_data` становится коротким и устойчивым (не завязано на длину путей).
- Кнопка “Назад” реализуется без FSM‑стека (через `parent_path`).
- UI редактирует одно сообщение и не засоряет чат.

Минусы/риски:
- Требуется предварительное наполнение SQLite (иначе навигация пустая).
- При смене структуры/переезде каталога нужна синхронизация, иначе UI будет показывать устаревшие данные.

Что нужно сделать:
- Добавить фоновые job’ы синхронизации и метку `catalog_last_sync_at` (таблица `meta`), чтобы UI мог работать исключительно по SQLite.

## Links
- Chatlog: `docs/CHATLOG_RU.md` (запись 2026-02-07 18:30 MSK)
- Related ADRs: ADR-005 (main‑only + repo hygiene), ADR-007 (фоновой sync каталога)
