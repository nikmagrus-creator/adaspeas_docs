...
Актуально на: 2026-02-05 16:45 MSK
...
- Устойчивость к ограничениям GitHub (trade controls/санкции/блокировки):
  - Remote может временно стать недоступен (SSH/HTTPS/вся платформа). В этом случае коммиты делаем как обычно локально.
  - Держим зеркальный remote (fallback) на альтернативном хостинге (например: GitLab/самостоятельный Gitea на VPS). Пушим туда до восстановления GitHub.
  - После восстановления GitHub делаем sync обратно в основной remote.
  - Если из‑за ограничений пришлось временно сделать репозиторий публичным, фиксируем это отдельной записью в `CHANGELOG.md` и в истории изменений этого контракта.
...
|---|---|---|---|---|
| 2026-02-05 19:00 MSK | ChatGPT | ci/docs | Исправлен CI-guard: разрешён только `.env.example`; добавлено правило про недоступность remote и временную публичность репозитория | |
