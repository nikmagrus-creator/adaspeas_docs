...
- VPS обновляется **только** через репозиторий (CI/CD). Ручные изменения на VPS запрещены.
- Любые изменения делаются коммитом и push в `main` (или через PR), после чего CI применит их на VPS.
