...
- Если нужен **полный архив** репозитория для передачи в чат (например, GitHub недоступен), делаем его **только** как снимок tracked‑файлов:
  - `git archive -o /media/nik/0C30B3CF30B3BE50/Загрузки/adaspeas.zip HEAD`
  - Такой zip **не содержит** `.git/`, ignored/untracked и не привязан к твоей машине.
- Запрещено пересылать “zip папки проекта” с вложенным `.git/`.
  - Если нужен перенос **истории** офлайн: используем `git bundle`, а не zip:
    - `git bundle create /media/nik/0C30B3CF30B3BE50/Загрузки/adaspeas.bundle --all`
...
|---|---|---|---|---|
| 2026-02-08 21:40 MSK | ChatGPT | docs | Зафиксировано: полный zip делаем через git archive (без .git); для офлайн-истории использовать git bundle | |
