...
Актуально на: 2026-02-05 19:40 MSK
...
## 0.2) Remote может временно стать недоступен (trade controls/санкции/блокировки)
- Считаем нормой, что любой провайдер (включая GitHub) может “внезапно” ограничить доступ к приватным репозиториям/Actions.
- Если GitHub недоступен или режет функционал: **коммиты делаем как обычно локально**, не ломаем историю.
- Держим **зеркальный remote (fallback)** на альтернативном хостинге (например, GitLab/Codeberg/самохост).
- После восстановления GitHub делаем **sync обратно**.
- Допустимо временно делать репозиторий **публичным** как обходной манёвр (если это единственный способ продолжать CI), но это должно быть зафиксировано в истории изменений и (при необходимости) в ADR по безопасности.
- Текущее состояние: репозиторий временно переведён в **public** (2026-02-05 MSK) из‑за trade controls, чтобы CI продолжал работать.
...
- любые изменения Docker/compose/CI считаются готовыми только если проходит job **ci-smoke** (подъём стека + /health)
...
| 2026-02-05 16:45 MSK | ChatGPT | contract | Добавлено правило: infra-изменения принимаются только при зелёном CI-smoke; CI усиливается hadolint+trivy+compose-smoke | |
| 2026-02-05 18:10 MSK | ChatGPT | ops/contract | Исправлен env-guard: разрешён .env.example; деплой workflow приведён к валидному YAML; добавлено правило про trade-controls и временную публичность репо | |
| 2026-02-05 19:40 MSK | ChatGPT | ci/deps/contract | CI: исправлен env-guard (не ругается на .env.example); deps: поднят aiogram для совместимости с aiohttp 3.13.x (CVE фикс); контракт: зафиксировано текущее public-состояние репо | |
