...
- Полные архивы (`adaspeas.zip`) также не кладём в репозиторий: они ломают проверку чистоты (`Repo dirty`) и подталкивают к ошибочному "распаковать поверх".
...
По умолчанию:
- ассистент выдаёт **только инкрементальный pack**;
- **полный архив** разрешён только если пользователь прямо просит (или если без него невозможно воспроизвести/починить состояние).
...
|---|---|---|---|---|
| 2026-02-07 20:10 MSK | ChatGPT | docs | Зафиксировано: по умолчанию только инкрементальные pack; полный архив только по явному запросу | |
