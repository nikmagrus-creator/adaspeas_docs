...
### 1.4) Правила сообщений коммитов (только русский)
- Сообщения коммитов пишем **только на русском**.
- Формат: `область: действие + уточнение` (в нижнем регистре, без точки в конце), например:
...
- Один коммит = одна смысловая правка. Смешивать “доки + код + инфраструктуру” без необходимости нельзя.

### 1.5) Минимум текста в чате
