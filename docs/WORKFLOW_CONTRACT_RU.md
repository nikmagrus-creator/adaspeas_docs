...
- Полные архивы (`adaspeas.zip`) также не кладём в репозиторий: они ломают проверку чистоты (`Репозиторий не чистый`) и подталкивают к ошибочному "распаковать поверх".
- В репозитории добавлен `.gitignore` для `adaspeas.zip`, `adaspeas_pack_*.tar.gz` и `.pack/`, но это страховка, а не приглашение хранить мусор рядом с кодом.
