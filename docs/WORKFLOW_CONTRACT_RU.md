...
- Если в одном чате передано несколько архивов/pack'ов, **актуален только последний по времени**. Всё, что выше по чату, считается устаревшим и не используется как вход.
...
Важно про артефакты pack/zip:
- Инкрементальные pack'и (.tar.gz) храним в папке скачивания (`/media/nik/0C30B3CF30B3BE50/Загрузки`), а не в корне репозитория.
- Полные архивы (например, `adaspeas_src_*.tar.gz` или старый `adaspeas.zip`) также не кладём в репозиторий: они ломают проверку чистоты (`Репозиторий не чистый`) и подталкивают к ошибочному "распаковать поверх".
- В репозитории добавлен `.gitignore` для `.pack/`, `adaspeas_pack_*.tar.gz`, `adaspeas_src_*.tar.gz`, `adaspeas.tar.gz`, `adaspeas.zip` и мусора копипаста (`:contentReference*`), но это страховка, а не приглашение хранить артефакты рядом с кодом.
...
- Если ассистент выдал pack и каноничный блок команд apply→commit→push, и пользователь не сообщил об ошибке, по умолчанию считаем, что патч применён. Возвращаемся к теме только при багрепорте или если новый архив показывает, что изменения не попали.
- Реплика пользователя `далее` означает «продолжай текущую задачу». Если ассистент написал, что продолжит/доделает в следующем сообщении и ещё не выдал pack (или явно не закрыл задачу), он продолжает ту же задачу и не переключается на другую.
- В одном pack/патче закрываем **максимум логически связанного**: фиксы + тесты + документация + гигиена в той же зоне. Не дробим одну проблему на 5 паков без причины.
- При этом не мешаем в один pack несвязанные темы (иначе потом больно откатывать и разбирать).
...
```
PRE-FLIGHT
- Режим: GitHub main | Архив (.tar.gz) | Инкрементальный pack | Web (внешние факты)
...
- **Сделать большие правки и не зависеть от состояния GitHub** → нужен **полный архив репозитория** или **инкрементальный pack** (актуальный).
  - Полный архив должен быть **снимком только tracked-файлов** (например, `deploy/make_ai_archive.sh` / `git archive`), без `.git/` и без игнорируемого мусора (`__pycache__/`, `*.pyc`, `.pytest_cache/` и т.п.). Если в архиве есть такие артефакты, архив считается неверным входом.
...
- Если сессия меняет процесс/архитектуру/инварианты: обновить `docs/HANDOFF_RU.md` для переезда в новый чат.
...
|---|---|---|---|---|
| 2026-02-10 23:20 MSK | ChatGPT | docs | PRE-FLIGHT: режим архива приведён к tar.gz (не zip) | |
| 2026-02-10 22:20 MSK | ChatGPT | docs | Синхронизировано: полный архив для чата = tar.gz (make_ai_archive / git archive --format), обновлены ссылки на zip | |
| 2026-02-10 16:10 MSK | ChatGPT | docs | Уточнено требование к полному архиву: только tracked-файлы (git archive), без __pycache__/pyc/...; обновлена актуальность | |
| 2026-02-08 22:30 MSK | ChatGPT | docs | Уточнено: в чате актуален только последний архив/pack; если ошибок не сообщено, патч считается применён; добавлена защита от :contentReference при копипасте | |
| 2026-02-07 19:45 MSK | ChatGPT | docs | Уточнено: артефакты pack/zip не держим в репо; ci-smoke принудительно включает CI_SMOKE=1; добавлен .gitignore для pack/.pack | |
