...
Дополнительно про админ‑команды (/sync и т.п.):
- Права проверяются по `from_user.id` (Telegram user id). В Bot API это поле называется `User.id` и является числовым уникальным идентификатором пользователя/бота.
- Чтобы узнать свой ID, используй команду `/id` у бота.
- В проде рекомендуется явно задавать `ADMIN_USER_IDS`, иначе админ‑команды будут доступны всем (если список пустой).
- Если бот отвечает «Недостаточно прав…», добавь свой ID в `ADMIN_USER_IDS` (через запятую) в `.env` и перезапусти сервисы (`make up-prod` на VPS или `docker compose up -d` локально).
