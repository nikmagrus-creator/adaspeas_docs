...
Ожидаемые сервисы:
- bot (aiogram): обрабатывает Telegram апдейты, отдаёт UI (и scheduler предупреждений об истечении доступа при включённом access control).
- worker: выполняет фоновые задачи (доставка файлов, синхронизация, админ‑уведомления о финальных ошибках).
- redis: очередь.
...
3) Если массовые 429/таймауты — временно выключить шумные задачи (предупреждения об истечении доступа в bot и/или синхронизацию каталога в worker), оставив базовый UI.
...
- предупреждениях об истечении доступа (если включён access control; выполняется в bot scheduler).
...
|---|---|---|---|---|
| 2026-02-11 00:05 MSK | ChatGPT | ops | Убрана коллизия: предупреждения об истечении доступа выполняет bot (не worker) | |
