...
- Если в одном чате загружено несколько архивов/pack-ов: использовать **только самый последний** как актуальный, всё предыдущее считать устаревшим.
- Если ассистент написал, что «продолжит/доделает дальше», но **не выдал pack**, то следующая реплика пользователя `далее` означает: **продолжай ту же задачу**, без переключения контекста.
- Если ассистент выдал pack + блок команд и пользователь не сообщил об ошибке, считаем, что pack применился корректно (исключение: позже проверка по свежему архиву показывает, что правки не доехали).
...
Админские команды:
- `/users` (админский UI) — список пользователей **с пагинацией и поиском**, использует `admin_sessions` в SQLite для короткого `callback_data` (`ul:<token>:<offset>`).
- `/search` — поиск по каталогу (FTS5), использует `search_sessions` для коротких кнопок (`ss:<token>:<offset>`).
