...
Актуально на: 2026-02-09 13:30 MSK
...
## 5) Единый pack для переезда

Если нужно перенести всё “одним махом” в новый чат, используй единый pack:
- `adaspeas_pack_2026-02-09_megapack_handoff_and_db_fix.tar.gz`

Он включает: идемпотентную миграцию v6 (`users.status`), helper `_executescript_idempotent`, retry DB init у worker, актуальные правила процесса, apply‑шаблон и обновлённые документы.
